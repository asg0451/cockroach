...
func newKafkaSinkV2Fx(t *testing.T, opts ...fxOpt) *kafkaSinkV2Fx {
	...
	var err error
	fx.sink, err = newKafkaSinkClientV2(ctx, fx.additionalKOpts, fx.batchConfig, uri, settings, knobs, nilMetricsRecorderBuilder, nil, changefeedbase.CreateKafkaTopicsAuto)
	...
	bs, err := makeKafkaSinkV2(ctx, &changefeedbase.SinkURL{URL: u}, targets, fx.sinkJSONConfig, 1, nilPacerFactory, timeutil.DefaultTimeSource{}, settings, nilMetricsRecorderBuilder, knobs, changefeedbase.CreateKafkaTopicsAuto)
	if err != nil && fx.createClientErrorCb != nil {
		fx.createClientErrorCb(err)
		return fx
	}
