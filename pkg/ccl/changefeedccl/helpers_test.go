...
func checkSchema(actual []cdctest.TestFeedMessage) error {
	for _, tm := range actual {
		...
		if err := gojson.Unmarshal(tm.Value, &msg); err != nil {
			return errors.Wrapf(err, `unmarshal: %s`, tm)
		}
